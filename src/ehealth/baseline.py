...
from ehealth.anntools import Collection, Keyphrase, Relation
...
class Baseline:
    ...
    def fit(self, path: Path):
        ...
        print(
            f"Training completed: Stored {len(keyphrases)} keyphrases and {len(relations)} relation pairs."
        )
...
def main():
    ...
    parser.add_argument(
        "--ref", required=True, type=Path, help="Location of the reference data."
    )
    parser.add_argument(
        "--eval", required=True, type=Path, help="Location of the evaluation data."
    )
    parser.add_argument(
        "--scenarios", type=int, nargs="+", help="Scenarios to run.", default=[1, 2, 3]
    )
    parser.add_argument(
        "--submit", required=True, type=Path, help="Location to output the submission."
    )

    args = parser.parse_args()

    baseline = Baseline()
    keyphrases, relations = baseline.fit(args.ref)
